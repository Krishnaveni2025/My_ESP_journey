...
//Display diagonal all LEDs connecting from left top to right bottom 
...
// Light LEDs from left top to right bottom  diagonal
for (unsigned char row = 1; row <= 8; row++) {
  sendData(row, 1 << (row - 1)); // Bit shifts increase with row number
}
}

void loop() {
  // No action needed; LEDs stay on statically
}
