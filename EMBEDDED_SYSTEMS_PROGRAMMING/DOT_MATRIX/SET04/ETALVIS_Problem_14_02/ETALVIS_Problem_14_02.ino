...
//use PORT A 
//self Assessment set -14 Problem 02
//Display LEFTmost top LED.
...
void setup() {
  ...
  *porta = (1 << CS_BIT); // CS HIGH initially, others LOW
  ...
  // Light rightmost top LED (row 1, column 8)
  sendData(0x01, 0b00000001); // Row 1, only bit 0 (column 8) is 1
}

void loop() {
  // No action needed; LED stays on statically
}

void delay1(void) {
  volatile long d;
  for (d = 0; d <= 300; d++); // Short delay for consistency
}

void delay2(void) {
  volatile long d;
  for (d = 0; d <= 500000; d++); // Longer delay (unused)
}
