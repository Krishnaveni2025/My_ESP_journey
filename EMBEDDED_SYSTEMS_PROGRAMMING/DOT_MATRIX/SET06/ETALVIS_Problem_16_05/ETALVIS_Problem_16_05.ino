...
void setup(void) {
    ...
    // 1️⃣ Entry from LEFT → over 8 steps × 250 ms = 2 seconds
    for (char step = 0; step <= 7; step++) {
        for (unsigned char row = 1; row <= 8; row++) {
            unsigned char m1 = 0, m2 = 0, m3 = 0, m4 = 0;

            if (step == 0)      { m1 = letterL[row - 1]; }                          // [L][][][]
            else if (step == 1) { m1 = letterA[row - 1]; m2 = letterL[row - 1]; }   // [A][L][][]
    ...
    // 2️⃣ Static hold for 10 seconds
    delay_ms(10000);

    // 3️⃣ Exit DOWNWARD → 8 steps × 500 ms = 4 seconds
