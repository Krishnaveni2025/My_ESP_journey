...
// === Display "ETAL" with Top-to-Bottom Entry, Static, and Right Shift Exit ===
void setup() {
  initMAX7219();

  // Entry: Shift letters in from the top (4 seconds, 8 steps)
  ...
  // Exit: Shift letters right one by one (3 seconds, 4 steps)
  for (char step = 0; step <= 4; step++) {
    for (unsigned char row = 1; row <= 8; row++) {
      ...
      if (step == 0) {
        ...
        m4_data = letterA[row - 1];
      } else if (step == 2) {
        m3_data = letterE[row - 1];
        m4_data = letterT[row - 1];
      } else if (step == 3) {
        m4_data = letterE[row - 1];
      }

      sendFourMatrices(9 - row, m1_data, m2_data, m3_data, m4_data);
    }
