...
// === Letter Definitions (Index 0 = Top, Index 7 = Bottom) ===
...
// === Display "ETAL" with Right-to-Left Entry, Static, and Bottom-to-Top Exit ===
void setup() {
  initMAX7219();

  // Entry: Shift letters in from the right (2 seconds, 8 steps)
  for (char step = 0; step <= 7; step++) {
    for (unsigned char row = 1; row <= 8; row++) {
      ...
      // Assign letters based on step
      if (step == 0) {
        m4_data = letterE[row - 1]; // [][][][E]
      } else if (step == 1) {
        m3_data = letterE[row - 1];
        m4_data = letterT[row - 1]; // [][E][T][]
      } else if (step == 2) {
        m2_data = letterE[row - 1];
        m3_data = letterT[row - 1];
        m4_data = letterA[row - 1]; // [E][T][A][]
      } else if (step >= 3) {
        m1_data = letterE[row - 1];
        m2_data = letterT[row - 1];
        m3_data = letterA[row - 1];
        m4_data = letterL[row - 1]; // [E][T][A][L]
      }

      sendFourMatrices(row, m1_data, m2_data, m3_data, m4_data);
    }
    delay_ms(250); // 2000 ms / 8 steps = 250 ms per step
  }

  // Static display for 10 seconds
  delay_ms(10000);

  // Exit: Shift letters up to top (4 seconds, 8 steps)
  for (char shift = 0; shift <= 7; shift++) {
    for (unsigned char row = 1; row <= 8; row++) {
      unsigned char e_data = (row - 1 + shift < 8) ? letterE[row - 1 + shift] : 0x00;
      unsigned char t_data = (row - 1 + shift < 8) ? letterT[row - 1 + shift] : 0x00;
      unsigned char a_data = (row - 1 + shift < 8) ? letterA[row - 1 + shift] : 0x00;
      unsigned char l_data = (row - 1 + shift < 8) ? letterL[row - 1 + shift] : 0x00;
