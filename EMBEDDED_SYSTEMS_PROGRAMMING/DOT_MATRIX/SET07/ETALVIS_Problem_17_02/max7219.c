...
void max7219_init(void) {
    ...
    *porta = (1 << CS_BIT);
    ...
    for (char i = 0; i < 4; i++) {
        ...
        max7219_send_row4(0x0B, 0x07, 0x07, 0x07, 0x07); // Scan limit
        max7219_send_row4(0x0F, 0x00, 0x00, 0x00, 0x00); // Shutdown off
    }

    for (unsigned char row = 1; row <= 8; row++)
        max7219_send_row4(row, 0x00, 0x00, 0x00, 0x00); // Clear screen
}